...
// --- Helper for Fast2SMS OTP Service ---
async function sendOtpViaFast2Sms(mobileNumber: string, otp: string): Promise<any> {
    const apiKey =  process.env.FAST2SMS_API_KEY;
    const senderId = process.env.FAST2SMS_SENDER_ID;
    const messageTemplateId = process.env.FAST2SMS_MESSAGE_TEMPLATE_ID;

    if (!apiKey || !senderId || !messageTemplateId) {
        console.error("Fast2SMS config not set. Set in Firebase environment (fast2sms.api_key, etc.) or local .env file (FAST2SMS_API_KEY, etc.).");
        throw new functions.https.HttpsError("failed-precondition", "The OTP service is not configured correctly. Please contact support.");
    }
    ...
    if (!response.ok || responseData.return !== true) {
        ...
        throw new functions.https.HttpsError("unavailable", `Fast2SMS Error: ${errorMessage}`, { apiResponse: responseData });
    }
...
// --- New Custom OTP Functions ---
export const sendCustomOtp = functions.https.onCall(async (data) => {
    const { mobileNumber, isSignup } = data;

    if (!/^[6-9]\d{9}$/.test(mobileNumber)) {
        throw new functions.https.HttpsError("invalid-argument", "Invalid 10-digit mobile number format.");
    }

    const usersRef = db.collection("users");
    const q = usersRef.where("mobileNumber", "==", mobileNumber).limit(1);
    const userSnapshot = await q.get();

    if (isSignup && !userSnapshot.empty) {
        throw new functions.https.HttpsError("already-exists", "This mobile number is already registered. Please login.");
    }
    if (!isSignup && userSnapshot.empty) {
        throw new functions.https.HttpsError("not-found", "This mobile number is not registered. Please sign up.");
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = admin.firestore.Timestamp.fromMillis(Date.now() + 5 * 60 * 1000); // 5 minutes expiry
    const otpRef = db.collection("otpStore").doc(mobileNumber);
...
export const verifyCustomOtp = functions.https.onCall(async (data) => {
    const { mobileNumber, otp, role, googleUser } = data; // Role is optional, only needed for signup

    if (otp === 'google-signup' && googleUser) {
        // --- Google Sign-up Flow ---
        const usersRef = db.collection("users");
        const q = usersRef.where("email", "==", googleUser.email).limit(1);
        const userSnapshot = await q.get();

        if (!userSnapshot.empty) {
             throw new functions.https.HttpsError("already-exists", "This email is already registered. Please login.");
        }
        
        let uid = googleUser.uid;
        // The user should already exist in Auth from the client-side popup
        const newUserDoc: AppUser = {
            uid: uid,
            email: googleUser.email,
            mobileNumber: googleUser.phoneNumber || '',
            role: role || 'client',
            createdAt: admin.firestore.FieldValue.serverTimestamp() as admin.firestore.Timestamp,
            name: googleUser.displayName || 'New User',
            ...(role === 'vendor' ? { isActive: false } : {}),
        };
    ...
    if (!/^[6-9]\d{9}$/.test(mobileNumber) || !/^\d{6}$/.test(otp)) {
        throw new functions.https.HttpsError("invalid-argument", "Invalid mobile number or OTP format.");
    }
    ...
    if (!otpDoc.exists) {
        throw new functions.https.HttpsError("not-found", "No OTP found for this number. Please request one again.");
    }
    ...
    if (userSnapshot.empty) {
        ...
        if (!role || (role !== 'client' && role !== 'vendor')) {
            throw new functions.https.HttpsError("invalid-argument", "A valid role is required for signup.");
        }
        
        const fullMobileNumber = `+91${mobileNumber}`;
        const newUserRecord = await admin.auth().createUser({ phoneNumber: fullMobileNumber });
        ...
        const newUserDoc: AppUser = {
            ...
            role: role,
            createdAt: admin.firestore.FieldValue.serverTimestamp() as admin.firestore.Timestamp,
            ...(role === 'vendor' ? { isActive: false } : { name: 'New User' }),
        };
        await db.collection("users").doc(uid).set(newUserDoc);
    } else {
        // Login Flow
